...
def receiveMidiBatch():
    ...
    for note in midiNotes:
        ...
        try:
            ...
            if code == 0b10000000:
                ...
                print("APPEND NOTE OFF: code ", format(code, '08b'), "eventType: ", eventType, "channel: ", channel,
                      "key: ", key, "vel: ", velocity, " time: ", time)
                prevTime = note["timestamp"]
            elif code == 0b10010000:
                eventType = "note_on"
                track.append(Message(eventType, channel=channel, note=key, velocity=velocity, time=time))
                print("APPEND NOTE ON: code ", format(code, '08b'), "eventType: ", eventType, "channel: ", channel,
                      "key: ", key, "vel: ", velocity, " time: ", time)
                prevTime = note["timestamp"]
                ...
                print("APPEND CONTROL CHANGE: code ", format(code, '08b'), "eventType: ", eventType, "channel: ",
                      channel, "key: ", key, "vel: ", velocity, " time: ", time)
                prevTime = note["timestamp"]
    ...
    ret = os.system(
        "{python} performance_alignment_workflow.py --smatPath {smatPath} --meiUri {meiUri} --structureUri  {structureUri} --solidContainer {solidContainer} --tpl-out {scriptsPath}/{uuid}-tplOut.jsonld --performanceMidiFile {scriptsPath}/{uuid}.mid --scoreUri {scoreUri} --audioContainer {audioContainer} --uuid {uuid}".format(
